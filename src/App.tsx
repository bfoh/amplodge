...
import { db, auth, initRealtimeUpdates } from '@/lib/db'
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged(async (state) => {
      if (!state.isLoading) {
        if (state.user?.id) {
          // Authenticated session — open the realtime channel so staff pages
          // refresh live. Anonymous visitors never reach this, so the public
          // site opens no websocket.
          initRealtimeUpdates().catch(() => {})
